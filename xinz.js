...
module.exports = async(xinz, msg, blocked, baterai, _afk, welcome, left) => {
    try {
        const { menu, newMenu, stickerMenu, ownerMenu, groupMenu, sistemMenu, gabutMenu, gameMenu, downloadMenu, searchMenu, stalkMenu, randomMenu, animeMenu, toolsMenu, makerMenu, otherMenu, hentaiMenu, storageMenu, premMenu } = require("./help");
     ...
     thumbnail: fs.readFileSync(setting.pathImg)
        ...
        // Anti link
        if (isGroup && isAntiLink && !isOwner && !isPremium && !isGroupAdmins && isBotGroupAdmins){
        ...
        // Anti wame
        if (isGroup && isAntiWame && !isOwner && !isPremium && !isGroupAdmins && isBotGroupAdmins){
        ...
        // Badword
        if (isGroup && isBadword && !isOwner && !isPremium && !isGroupAdmins){
            for (let kasar of badword){
                if (chats.toLowerCase().includes(kasar)){
                    if (isCountKasar(sender, senbadword)){
                        if (!isBotGroupAdmins) return textImg(`Kamu beruntung karena bot bukan admin😌\nKalo admin dah bot kick karena kamu berkata kasar`)
                        textImg(`*「 ANTI BADWORD 」*\n\nSepertinya kamu sudah berkata kasar lebih dari 5x, maaf kamu akan di kick`)
                        xinz.groupRemove(from, [sender])
                        delCountKasar(sender, senbadword)
                    } else {
                        addCountKasar(sender, senbadword)
                        textImg(`Kamu terdeteksi berkata kasar🤬\nKamu akan di kick jika berkata kasar lebih dari 5x. Jangan ulangi lagi❗`)
                    }
                }
            }
        }
        ...
        if (isMuted){
            ...
            if (chats.toLowerCase().startsWith(prefix+'unmute')){
                ...
                textImg(`Bot telah diunmute di group ini`)
            }
        }
        ...
        if (game.isTebakGambar(from, tebakgambar) && isUser){
            if (chats.toLowerCase().includes(game.getJawabanTG(from, tebakgambar))){
                ...
                await textImg(`*Selamat jawaban kamu benar*\n*Jawaban :* ${game.getJawabanTG(from, tebakgambar)}\n*Hadiah :* $${htgm}\n\nIngin bermain lagi? kirim *${prefix}tebakgambar*`)
            ...
            if (mentioned.length !== 0){
                for (let ment of mentioned) {
                    if (afk.checkAfkUser(ment, _afk)) {
                        ...
                        sendMess(ment, `Ada yang mencari kamu saat sedang afk\n\nNama : ${pushname}\nNomor : wa.me/${sender.split("@")[0]}\nIn Group : ${groupName}\nPesan : ${chats}`)
                    }
                }
            }
        ...
        switch(command){
            ...
            case prefix+'allmenu':{
                ...
                textImg(menu(ucapan, setting.ownerName, setting.botName, prefix, pendaftar, runtime(process.uptime()), pushname, isOwner, isPremium, sisalimit, limitCount, sisaGlimit, gcount, expiredPrem(), tanggal, jam, setting.emote))
            })
            }
			break
            ...
            case prefix+'storagemenu': case prefix+'storage': case prefix+'menustorage':{
                ...
                break
			case prefix+'premiummenu': case prefix+'premmenu': case prefix+'menupremium': case prefix+'menuprem':{
                textImg(premMenu(prefix, setting.ownerName))
            }
                break
//------------------< Sticker / Tools >-------------------
            case prefix+'exif':{
            ...
            case prefix+'stickerwm': case prefix+'swm': case prefix+'take': case prefix+'takesticker': case prefix+'takestick':{
				...
				if (!isOwner && !isPremium) return textImg(mess.OnlyPrem)
            ...
            case prefix+'tomedia':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				if (!isQuotedSticker) return textImg('Reply stiker nya')
                let encmedia = JSON.parse(JSON.stringify(msg).replace('quotedM','m')).message.extendedTextMessage.contextInfo
				...
				if (quotedMsg.stickerMessage.isAnimated === true){
                    ...
                    exec(`convert ${media} ${outGif}`, (err) => {
                        ...
                        exec(`ffmpeg -i ${outGif} -vf "crop=trunc(iw/2)*2:trunc(ih/2)*2" -b:v 0 -crf 25 -f mp4 -vcodec libx264 -pix_fmt yuv420p ${outMp4}`, (err) => {
                            if (err) {
                                ...
                                return textImg(`Error`)
                            }
                            xinz.sendVideo(from, fs.readFileSync(outMp4), 'Nih, Jangan lupa FOLLOW https://instagram.com/ciko.b7 ya kak >///<', msg)
                            .then(() => {
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						xinz.sendMessage(from, fs.readFileSync(ran), image, {quoted: msg, caption: 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<'})
                        limitAdd(sender,  limit)
						fs.unlinkSync(ran)
					})
					}
            ...
            case prefix+'tinyurl':
                ...
                if (!isUrl(args[1])) return textImg(`Masukkan link yang benar`)
                axios.get(`https://tinyurl.com/api-create.php?url=${args[1]}`)
                .then((a) => textImg(`Nih ${a.data}`))
                .catch(() => textImg(`Error, harap masukkan link dengan benar`))
                break
            case prefix+'imgtourl':{
                if (isImage || isQuotedImage) {
                    ...
                    await textImg(`${upload.image.image.url}`)
                    limitAdd(sender, limit)
                } else {
            ...
            .catch(() => textImg(`Apikey invalid`))
            }
            break
            case prefix+'ebase64':
                if (!q) return textImg(`Format salah!\n\nKirim perintah: ${prefix}ebase64 *text*\nContoh: ${prefix}ebase64 helloworld`)
                if (q.length > 2048) return textImg('Maximal 2.048 String!')
...
//------------------< NULIS >--------------------- 
            ...
            case prefix+'nuliskiri':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}nuliskiri* teks`)
                textImg(mess.wait)
                ...
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkiri.jpg'), image, {quoted: msg, caption: 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', thumbnail: Buffer.alloc(0)})
                    limitAdd(sender, limit)
                })
            }
                break
            case prefix+'nuliskanan':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}nuliskanan* teks`)
                textImg(mess.wait)
                ...
                .on('error', () => textImg(mess.error.api))
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkanan.jpg'), image, {quoted: msg, caption: 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', thumbnail: Buffer.alloc(0)})
                    limitAdd(sender, limit)
                })
            }
                break
            case prefix+'foliokiri':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}foliokiri* teks`)
                textImg(mess.wait)
                const tulisan = body.slice(11)
                ...
                .on('error', () => textImg(mess.error.api))
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkiri.jpg'), image, {quoted: msg, caption: 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', thumbnail: Buffer.alloc(0)})
                    limitAdd(sender, limit)
                })
            }
                break
            case prefix+'foliokanan':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}foliokanan* teks`)
                textImg(mess.wait)
                const tulisan = body.slice(12)
                const splitText = tulisan.replace(/(\S+\s*){1,13}/g, '$&\n')
                const fixHeight = splitText.split('\n').slice(0, 38).join('\n')
                ...
                .on('error', () => textImg(mess.error.api))
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkanan.jpg'), image, {quoted: msg, caption: 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', thumbnail: Buffer.alloc(0)})
                    limitAdd(sender, limit)
                })
            }
                break
...
//------------------< Text Marker >-------------------
            case prefix+'harta': case prefix+'hartatahta': case prefix+'tahta':{
                ...
                textImg(mess.wait)
                xinz.sendImage(from, await getBuffer(`https://api-ramlan.herokuapp.com/api/other/tahta?q=${args[1]}&apikey=${apikey}`), '', msg).catch(() => textImg(mess.error.api))
                limitAdd(sender, limit)
                }
                break
//>>> TEXT PRO ME
                ...
                if (args.length < 2) return textImg(`Penggunaan ${command} text\n\nContoh : ${command} xinz`)
                textImg(mess.wait)
                xinz.sendImage(from, await getBuffer(`https://api-ramlan.herokuapp.com/api/textpro/${command.slice(1)}?apikey=${apikey}&text=${q}`), 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg).catch(() => textImg(mess.error.api))
                limitAdd(sender, limit)
                }
                    break
                ...
                textImg(mess.wait)
                xinz.sendImage(from, await getBuffer(`https://api-ramlan.herokuapp.com/api/textpro2/${command.slice(1)}?apikey=${apikey}&text1=${q.split("|")[0]}&text2=${q.split("|")[1]}`), 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg).catch(() => textImg(mess.error.api))
                limitAdd(sender, limit)
                }
                    break
//>>> PHOTOOXY
                ...
                if (args.length < 2) return textImg(`Penggunaan ${command} text\n\nContoh : ${command} xinz`)
                textImg(mess.wait)
                xinz.sendImage(from, await getBuffer(`https://api-ramlan.herokuapp.com/api/photooxy/${command.slice(1)}?apikey=${apikey}&text=${q}`), 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg).catch(() => textImg(mess.error.api))
                limitAdd(sender, limit)
                }
                    break
                ...
                if (!q.includes("|")) return textImg(`Penggunaan ${command} text1|text2\n\nContoh : ${command} Ramlan|Rara`)
                textImg(mess.wait)
                xinz.sendImage(from, await getBuffer(`https://api-ramlan.herokuapp.com/api/photooxy/pubglogo?apikey=${apikey}&text1=${q.split("|")[0]}&text2=${q.split("|")[1]}`), 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg).catch(() => textImg(mess.error.api))
                limitAdd(sender, limit)
                }
                    break
//------------------< Math Random >-------------------
				...
				case prefix+'cantik':
					...
					case prefix+'putus':
					if (!isGroup)return textImg(mess.OnlyGrup)
					var kamu1 = groupMembers
					var cinta1 = groupMembers
					var aku1 = cinta1[Math.floor(Math.random() * kamu1.length)]
					var cintax1 = kamu1[Math.floor(Math.random() * cinta1.length)]
					let vejs1 = `Kasian ada yang habis putus🤭\n*@${aku1.jid.split('@')[0]}* 💔 @${cintax1.jid.split('@')[0]}\nSemoga Dapet Pacar Baru`
					mentions(vejs1, [aku1.jid, cintax1.jid], true)
					break
				case prefix+'seberapagay':
				if (!isGroup)return textImg(mess.OnlyGrup)
                if (!q) return textImg(`Penggunaan ${command} text\n\nContoh : ${command} Rara`)
...
//------------------< Random >---------------------
				...
				case prefix+'darkjokes': case prefix+'darkjoke': case prefix+'jokes': case prefix+'dark':{
					...
					.then(({data}) => {
					sendFileFromUrl(from, data.urlimage, 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg)
					limitAdd(sender, limit)
					})
				}
				break
...
//------------------< Baileys >---------------------
            ...
            case prefix+'kontak':
                ...
                break
            case prefix+'hidetag':{
                if (!isGroupAdmins && !isOwner && !isPremium) return textImg(mess.OnlyPrem)
...
//------------------< Downloader >-------------------
            case prefix+'ytmp4':{
                ...
                if (!isLinks2) return textImg(mess.error.Iv)
                try {
                    textImg(mess.wait)
                    ytv(args[1])
                    .then((res) => {
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
            ...
            case prefix+'ytmp3':{
                ...
                if (!isLinks) return textImg(mess.error.Iv)
                try {
                    textImg(mess.wait)
                    yta(args[1])
                    .then((res) => {
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
            ...
            case prefix+'playmp4':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length === 1) return textImg(`Kirim perintah *${prefix}playmp4* judul_video`)
                try {
                    textImg(mess.wait)
                    let yut = await yts(q)
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
            ...
            case prefix+'play': case prefix+'playmp3':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length === 1) return textImg(`Kirim perintah *${prefix}play* judul_lagu`)
                try {
                    textImg(mess.wait)
                    let yut = await yts(q)
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
            ...
            case prefix+'ig': {
                ...
                if (!isUrl(args[1]) && !args[1].includes('instagram.com')) return textImg(mess.error.Iv)
                textImg(mess.wait)
            ...
            case prefix+'facebook':{
                ...
                if (!isUrl(args[1]) && !args[1].includes('facebook.com')) return textImg(mess.error.Iv)
                textImg(mess.wait)
                fbdl(args[1])
                .then((res) => {
...
*Hasil Pencarian : ${q}*\n`
               ...
               case prefix+'tiktok': case prefix+'tiktoknowm': {
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return textImg(`Penggunaan ${command} _link tiktok_\n\nContoh : ${command} https://vt.tiktok.com/ZSJVPawwv/`)
                if (!isUrl(args[1]) && !args[1].includes('tiktok.com')) return textImg(body.replace(args[1], "*"+args[1]+"*")+'\n\n'+mess.error.Iv+`\nContoh : ${command} https://vt.tiktok.com/ZSJVPawwv/`)
                textImg(mess.wait)
...
\`\`\`▢ Bio :\`\`\`\n${bio}`
            ...
            case prefix+'pinterest':
                ...
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return textImg(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                //if (!isPremium) return textImg(mess.OnlyPrem)
			if (args.length === 1) return textImg(`Kirim perintah *${prefix}pinterest query*`)
                ...
                pinterest(q).then(async(res) => {
                    ...
                    await sendFileFromUrl(from, res.image, `Hasil Pencarian: ${q}\nJangan lupa FOLLOW https://Instagram.com/harits_fahrizal ya kak >///<`, msg)
                }).catch(async(err) => {
            ...
            case prefix+'readmore':
                ...
                textImg(`${rmoreteks1}${readMore}${rmoreteks2}`)
                break
//------------------< VVIBU >-------------------
			case prefix+'waifu':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                textImg(mess.wait)
						...
						.then(({data}) => {
						sendFileFromUrl(from, data.link, 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg)
						})
						limitAdd(sender, limit)
						}
                      break
			...
			case prefix+'megumin':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                textImg(mess.wait)
						sendFileFromUrl(from, `https://api-ramlan.herokuapp.com/api/megumin?apikey=${apikey}`, 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg)
						limitAdd(sender, limit)
						}
                      break
			case prefix+'sagiri':{
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                textImg(mess.wait)
						sendFileFromUrl(from, `https://api-ramlan.herokuapp.com/api/sagiri?apikey=${apikey}`, 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg)
						limitAdd(sender, limit)
						}
                      break
			case prefix+'shinobu':{
                ...
                textImg(mess.wait)
						...
						.then(({data}) => {
						sendFileFromUrl(from, data.url, 'Nih, Jangan lupa FOLLOW https://instagram.com/harits_fahrizal ya kak >///<', msg)
						})
						limitAdd(sender, limit)
						}
                      break
...
//------------------< nsfw >-------------------
            ...
            case prefix+'trap':{
                ...
                if (!isOwner && !isPremium) return textImg(mess.OnlyPrem)
			if (!isGroup)return textImg(mess.OnlyGrup)
			if (!isNsfw) return textImg('Nsfw group belum aktif')
                textImg(mess.wait)
...
//------------------< BAN >-------------------
            case prefix+'ban':
                ...
                if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender === ownerNumber) return textImg(`Tidak bisa ban Owner`)
                    ...
                    textImg(`Kirim perintah ${prefix}ban @tag atau nomor atau reply pesan orang yang ingin di ban`)
                }
                break
            case prefix+'unban':
                if (!isOwner) return textImg(mess.OnlyOwner)
                if (mentioned.length !== 0){
                    ...
                    textImg(`Kirim perintah ${prefix}unban @tag atau nomor atau reply pesan orang yang ingin di unban`)
                }
                break
            case prefix+'listban':
...
//------------------< Game >-------------------
            ...
            case prefix+'buylimit':{
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}buylimit* jumlah limit yang ingin dibeli\n\nHarga 1 limit = $150 balance`)
                ...
                textImg(monospace(`Pembeliaan limit sebanyak ${args[1]} berhasil\n\nSisa Balance : $${getBalance(sender, balance)}\nSisa Limit : ${getLimit(sender, limitCount, limit)}/${limitCount}`))
            }
                break
            ...
            case prefix+'buyglimit':{
                ...
                textImg(monospace(`Pembeliaan game limit sebanyak ${args[1]} berhasil\n\nSisa Balance : $${getBalance(sender, balance)}\nSisa Game Limit : ${cekGLimit(sender, gcount, glimit)}/${gcount}`))
            }
                break
            case prefix+'tictactoe': case prefix+'ttt': case prefix+'ttc':
                if (!isGroup)return textImg(mess.OnlyGrup)
                if (isGame(sender, isOwner, gcount, glimit)) return textImg(`Limit game kamu sudah habis`)
                if (isTicTacToe(from, tictactoe)) return textImg(`Masih ada game yg blum selesai`)
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}tictactoe* @tag`)
                if (mentioned.length !== 0){
						if (mentioned[0] === sender) return textImg(`Sad amat main ama diri sendiri`)
                    ...
                    textImg(`Kirim perintah *${prefix}tictactoe* @tag`)
                }
                break
            case prefix+'delttt':
            case prefix+'delttc':
                if (!isGroup)return textImg(mess.OnlyGrup)
                if (!isTicTacToe(from, tictactoe)) return textImg(`Tidak ada sesi game tictactoe di grup ini`)
                tictactoe.splice(getPosTic(from, tictactoe), 1)
                textImg(`Berhasil menghapus sesi tictactoe di grup ini`)
                break
            case prefix+'suit':
                if (isGame(sender, isOwner, gcount, glimit)) return textImg(`Limit game kamu sudah habis`)
                if (args.length < 2) return textImg(`Penggunaan ${command} gunting/kertas/batu\n\nContoh : ${command} gunting`)
                ...
                if (isSuit){
                    ...
                    textImg(body.replace(args[1], "*"+args[1]+"*")+'\n\n'+`Tidak ada pilihan ${args[1]}`+`\nContoh : ${command} gunting`)
                }
                break
...
//------------------< Owner >-------------------
            ...
            case prefix+'setthumb':
                if (!isOwner) return textImg(mess.OnlyOwner)
                    if (!isQuotedImage) return textImg('Reply imagenya blokk!')
...
//------------------< G R U P >-------------------
			...
			case prefix+'d':
				if (!isGroup)return textImg(mess.OnlyGrup)
				if (!isGroupAdmins && !isOwner && !isPremium)return textImg(mess.GrupAdmin)
            ...
            case prefix+'afk':
                if (!isGroup) return textImg(mess.OnlyGrup)
                if (isAfkOn) return textImg('afk sudah diaktifkan sebelumnya')
                if (body.slice(150)) return textImg('Alasanlu kepanjangan')
			...
			//textImg('Maaf fitur ini di matikan karena menjadi penyebab nomor bot terblokir oleh WhatsApp')
		    ...
		    if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender === ownerNumber) return textImg(`Tidak bisa kick Owner`)
            ...
            case prefix+'group':
                ...
                if (!isGroupAdmins && !isOwner)return textImg(mess.GrupAdmin)
                if (!isBotGroupAdmins) return textImg(mess.BotAdmin)
                if (args.length === 1) return textImg(`Pilih open atau close\nContoh : ${prefix}group close`)
                if (args[1].toLowerCase() === 'open'){
                    xinz.groupSettingChange(from, "announcement", false)
                mentions(`Group telah di *buka* oleh admin @${sender.split('@')[0]}`, [sender], true)
                } else if (args[1].toLowerCase() === 'close'){
                    xinz.groupSettingChange(from, "announcement", true)
                mentions(`Group telah di *tutup* oleh admin @${sender.split('@')[0]}`, [sender], true)
                } else {
                    textImg(`Pilih open atau close\nContoh : ${prefix}group close`)
                }
                break
            case prefix+'setppgrup':
                ...
                break
            case prefix+'join':
			if (!isPremium && !isOwner) return textImg(mess.OnlyPrem)
                //if (!isOwner) return textImg(mess.OnlyOwner)
                if (args.length < 2) return textImg(`Kirim perintah *${prefix}join* link grup`)
                if (!isUrl(args[1]) && !args[1].includes('chat.whatsapp.com')) return textImg(mess.error.Iv)
...
//------------------< Enable / Disable >-------------------
            case prefix+'antibadword':
                ...
                if (args[1].toLowerCase() === 'enable'){
					...
					textImg(`antibadword grup aktif, kirim ${prefix}listbadword untuk melihat list badword`)
                } else if (args[1].toLowerCase() === 'disable'){
                    let anu = grupbadword.indexOf(from)
                    grupbadword.splice(anu, 1)
                    fs.writeFileSync('./database/grupbadword.json', JSON.stringify(grupbadword))
                    textImg('antibadword grup nonaktif')
                } else {
            ...
            case prefix+'nsfw':
                ...
                if (args[1].toLowerCase() === 'enable'){
                    if (isNsfw) return textImg(`Udah aktif`)
                    nsfw.push(from)
            ...
            case prefix+'antilink':
                ...
                if (args[1].toLowerCase() === 'enable'){
					...
					textImg('Antilink grup aktif')
                } else if (args[1].toLowerCase() === 'disable'){
                    let anu = antilink.indexOf(from)
                    antilink.splice(anu, 1)
                    fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
                    textImg('Antilink grup nonaktif')
                } else {
                    textImg(`Pilih enable atau disable\nContoh : ${prefix}antilink enable`)
                }
                break
            case prefix+'antiwame':
                ...
                if (args[1].toLowerCase() === 'enable'){
					...
					textImg('Anti wa.me grup aktif')
                } else if (args[1].toLowerCase() === 'disable'){
                    let anu = antiwame.indexOf(from)
                    antiwame.splice(anu, 1)
                    fs.writeFileSync('./database/antiwame.json', JSON.stringify(antiwame))
                    textImg('Anti wa.me grup nonaktif')
                } else {
                    textImg(`Pilih enable atau disable\nContoh : ${prefix}antiwame enable`)
                }
                break
			...
			break
			case prefix+'cikzgroup':
case prefix+'cikzgrup':
const V1 = 'https://chat.whatsapp.com/CGSp4iyGWtf2ofWJilH1R3'
textImg(`LINK GROUP V1 : ${V1}\n\nJangan lupa untuk bergabung ya kak ${pushname} biar grup nya rame, hehe...`)
break
			case prefix+'stickergif':
			...
			case prefix+'s':{
                ...
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(msg).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : msg
                    let media = await xinz.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
					exif.create(`BY ROYAL BOT`, `wa.me/6288221813178`, `stickwm_${sender}`)
                    await ffmpeg(`${media}`)
							.input(media)
							.on('start', function (cmd) {
								console.log(`Started : ${cmd}`)
                    ...
                    let encmedia = JSON.parse(JSON.stringify(msg).replace('quotedM','m')).message.extendedTextMessage.contextInfo
				    let media = await xinz.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
                    exif.create(`BY ROYAL BOT`, `wa.me/6288221813178`, `takestick_${sender}`)
                    exec(`webpmux -set exif ./sticker/takestick_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                        if (error) return textImg(mess.error.api)
                        xinz.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: msg})
                        fs.unlinkSync(media)
                    ...
                    textImg(`Kirim gambar/gif dengan caption ${prefix}sticker atau tag gambar/gif yang sudah dikirim`)
                }
            }
                break
			case prefix+'daftarprem':
			case prefix+'daftarpremium':
textImg(`Hai kak mau daftar premium?Murah kok cuma 5K kamu udah bisa akses fitur-fitur menarik\n\nCek fitur fitur yang bisa kamu akses di ${prefix}premiummenu\n\n+Kebal *!antilink !antibadword !antiwame* dan Unlimited limit request🔥\n\n*Minat?* Silahkan ketik *!owner*`)
break
			case prefix+'tes':
			case prefix+'bot':
    ...
    break
			...
			case 'bot':
			case 'bott':
			textImg('Iya? Ada yang bisa Bot bantu? Untuk selengkapnya, Silahkan ketik *#Menu*')
			break
        }
    } catch (err) {
